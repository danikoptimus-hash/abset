...
>
> **Примечание (Admin monitoring panel):** новая таблица `monitoring_snapshots` (raw/hourly resource-usage история — память backend-процесса, размер БД, размер каталога данных, свободное место на диске; retention 24ч raw / 90 дней hourly) + колонка `jobs.peak_memory_mb` (пиковая память backend-процесса за время job'а). Без Docker-сокета и без новых сервисов — отдельный демон-поток в backend-процессе. Подробности — [docs/OPERATIONS.md](docs/OPERATIONS.md) §«Мониторинг».
...
| Смотреть панель мониторинга ресурсов                          |        |        |   ✓   |
