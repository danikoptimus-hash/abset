...
Техническое задание для реализации поверх текущего состояния проекта abkit (Streamlit-приложение + библиотека). Цель — превратить локальный инструмент в командный сервис, который разворачивается так же просто, как Apache Superset: `git clone` → `docker compose up` → работает.
