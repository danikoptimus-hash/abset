...
- **"Test connection"** в форме подключения выполняет `SELECT 1` и показывает
  человеческий результат (успех / хост недоступен / неверные учетные данные /
  база не найдена), таймаут 5-10 секунд.
...
секунд). SQL-датасеты можно обновить кнопкой **Refresh** в списке датасетов —
запрос выполняется заново, файл и `fetched_at` обновляются.
