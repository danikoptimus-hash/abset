...
(д) **Публикация релиза — `scripts/release.sh vX.Y.Z` на рабочей машине.**
    Прод-VM (CLK2-ABSET-01) не имеет выхода в интернет и не собирает образы
    вовсе: она тянет готовые из внутреннего Harbor и код из внутреннего
    GitLab. Отсюда разделение файлов: `docker-compose.yml` остается
    dev-дефолтом со сборкой из контекстов (у разработчика ничего не
    изменилось), `docker-compose.prod.yml` — оверлей, который подменяет
    сборку на `image: ${ABKIT_REGISTRY}/<name>:${ABKIT_VERSION}` и снимает
    `build:` через `!reset null` (иначе VM без интернета попыталась бы
    собирать вместо честного отказа на недоступном реестре). Что именно
    запущено на проде, определяет строка `ABKIT_VERSION` в `.env`, а НЕ
    `git checkout`: обновление и откат — это правка одной строки + `pull` +
    `up`, не зависящие ни от сети, ни от состояния рабочей копии.

    nginx получил собственный образ (`docker/nginx.Dockerfile`) вместо голого
    `nginx:alpine` с bind-mount'ом конфига — иначе версия прокси определялась
    бы состоянием рабочей копии на VM, и `git pull` менял бы поведение мимо
    ABKIT_VERSION. Поведение идентично: официальный образ сам прогоняет
    envsubst по `/etc/nginx/templates/*`, так что `ABKIT_MAX_UPLOAD_MB`
    по-прежнему применяется в рантайме.

    Быстрый гейт внутри release.sh (pyflakes + typecheck/lint + юнит-подмножество)
    — НЕ замена полному прогону: полный прогон это гейт ПРИЕМКИ ПАКЕТА, до
    коммита; здесь задача поймать очевидно сломанный релиз за минуту, до
    десятиминутной сборки трех образов. Проверка всего конвейера без
    настоящего Harbor — `scripts/test_release_local_registry.sh` (поднимает
    `registry:2`, гоняет release.sh с выбрасываемой версией и в конце
    ПОДНИМАЕТ прод-оверлей из вытянутых образов). Регламент, чек-лист первого
    развертывания и запасной путь «собрать на хосте» — docs/OPERATIONS.md
    §2-4 и Приложение А.
...
**Обвязка каскада: onSchemaChange получает СЕТТЕР, а не стрелку** (баг-фикс
после SQL Lab). `SchemaTableCascade` на выбор схемы вызывает ДВА обработчика
подряд — `onSchemaChange(value)`, затем `onTableChange(undefined)`. Пока
родитель передает голые сеттеры (`onSchemaChange={setSchema}`), это безопасно.
SQL Lab передавал вместо них один общий обработчик, восстанавливавший вторую
половину состояния из замыкания (`(next) => onCascadeChange(schema, next)`), —
и второй вызов записывал в схему УСТАРЕВШЕЕ (еще `undefined`) значение того же
рендера, отменяя первый: схема сбрасывалась в тот же тик, запрос таблиц не
уходил вовсе, пикер таблиц был пуст навсегда. Важно: компонент при этом
импортировался ОБЩИЙ — «переиспользовать компонент» оказалось необходимым, но
недостаточным условием, дублировалась не разметка, а обвязка. Гард —
`frontend/src/components/datasets/schemaBrowserReuse.test.ts` (сканирует весь
`pages/`+`components/`: ровно один файл владеет `aria-label`'ами пикеров, и у
каждого потребителя `onSchemaChange` — идентификатор, а не стрелка).

**Служебные схемы и схема по умолчанию** — фильтр и выбор `public`/`dbo`/
`default` живут на СЕРВЕРЕ (`introspection.py::visible_schemas`/
`default_schema`, ответ `/schemas` несет `default_schema`), а не в компоненте:
набор служебных имен — свойство движка (postgres: `information_schema` + весь
зарезервированный префикс `pg_`; mssql: `sys`/`guest`/`INFORMATION_SCHEMA` +
роли `db_*`; clickhouse: `system`/`INFORMATION_SCHEMA`), а движок известен
только на сервере — в React пришлось бы протаскивать `engine` через три формы
и держать три копии списка. Фильтр применяется ДО записи в кэш, чтобы
`?refresh=true` не приносил другой набор. Автовыбор схемы по умолчанию —
опциональный проп `autoSelectDefaultSchema` (включен в создании датасета и SQL
Lab, ВЫКЛЮЧЕН в Edit: там пустая схема означает «запрос написан руками», и
подставленный `public` врал бы про источник данных).
